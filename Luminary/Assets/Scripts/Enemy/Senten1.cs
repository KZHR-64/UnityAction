...
//ステージ2ボス、穿天
public class Senten1 : AbstructEnemy
{
    ...
    private IEnumerator StraightShoot()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            bulletManager.CreateBullet(bulletList[0], boneList[0].position, transform.rotation, 12.0f, transform); //弾を生成   
    ...
    private IEnumerator DashAttack()
    {
        ...
        xSpeed = 12.0f * transform.localScale.x; //速度を設定
        yield return new WaitForSeconds(0.5f); //少し待機
        xSpeed = 0.0f; //速度を設定
        animator.Play(standAnimHash); //アニメーションを再生
        yield return new WaitForSeconds(1.0f); //少し待機
